...
class Plotting:
    ...
    def xy_simulation_plot(self, x, xmin, xmax, x_label, y1, y2, y_label, y_legend1, y_legend2, COLOR1, COLOR2, ymin = -10, ymax = 16):
        fig, ax = plt.subplots()
        x = x/12
    ...
    def xy_simulation_pygf_plot(self, x, y1, COLOR1):
        ...
        ax.axhline(y = 0, color = 'black', linestyle = '-.', linewidth=0.3)        
        ax.legend()
        ax.set(xlabel=("Timer"), ylabel=("Gjennomsnittlig kollektorvæsketemperatur [°C]"), xlim=(0, len(x)), ylim=(-2,16))
        ax.grid(color='black', linestyle='--', linewidth=0.1)
        st.pyplot(plt)
